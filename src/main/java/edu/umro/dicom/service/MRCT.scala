...
class MRCT {
    ...
    private val channelLifetime: Long = 60 * 60 * 1000

    class Chan(val channel: Channel, val expirationTime: Long);

    var chan = new Chan(null, System.currentTimeMillis - 1)

    private def initChannel(force: Boolean): Unit = {
        if (force || (chan == null) || (chan.expirationTime < System.currentTimeMillis)) {
            Log.get.info("Constructing new channel.  Using AMQP broker at " + ServiceConfig.getInstance.getAmqpBrokerHost + ":" + ServiceConfig.getInstance.getAmqpBrokerPort)
            try {
                ...
                chan = new Chan(connection.createChannel, System.currentTimeMillis+channelLifetime)

                try chan.channel.exchangeDeclare(config.dataExchange, "topic", false)
    ...
    private class RecvDicom extends Object with ReceiveDicom {
        ...
        private def sendToDicomToCoordinator(eventText: Array[Byte], attributeList: AttributeList): Unit = {
            try {
                ...
                chan.channel.basicPublish(config.dataExchange, config.dataRoutingKey, null, bout.toByteArray)
            } catch {
                case e: Exception => {
                    Log.get.warning("Unable to send DICOM file to MR CT Coordinator: " + e + "\nEvent: " + new String(eventText))
                    Thread.sleep(10 * 1000)
                    Log.get.info("Attempting AMQP channel reset ...")
                    initChannel(true)
